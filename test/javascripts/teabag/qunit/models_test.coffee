...
test "constructor", 7, ->
  ...
  ok(spec.link == "?grep=_full%20qunit%20name_%20_description_", "sets link")
...
test "#errors", 3, ->
  errors = new Teabag.Spec(@mockSpec).errors()
...
test "#result", 3, ->
  @mockSpec.failed = 0
  result = new Teabag.Spec(@mockSpec).result()
