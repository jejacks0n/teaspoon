...
describe Teaspoon::Drivers::SlimerjsDriver do

  describe "#initialize" do

    it "assigns @options" do
      subject = Teaspoon::Drivers::SlimerjsDriver.new(foo: "bar")
      expect(subject.instance_variable_get(:@options)).to eq(["--foo=bar"])
    end

    it "accepts a string for options" do
      subject = Teaspoon::Drivers::SlimerjsDriver.new("--foo=bar --bar=baz")
    ...
    it "raises a Teaspoon::UnknownDriverOptions exception if the options aren't understood" do
      expect { Teaspoon::Drivers::SlimerjsDriver.new(true) }.to raise_error(Teaspoon::UnknownDriverOptions)
    end
  ...
  describe "#run_specs" do
    ...
    context "with slimerjs" do
      ...
      it "calls #run and calls runner.process with each line of output" do
        ...
        args = ["--foo", "--bar", Teaspoon::Engine.root.join("lib/teaspoon/drivers/slimerjs/runner.js").to_s, "_url_", 180]
